...
{
    public static Dictionary<string, string> drawTemplates = new();

    [Command("draw help [template]")]
    public static void DrawHelp(GroupMessageEventArgs e, string template = null)
    {
        string templates = "";
        if (template != null)
        {
    ...
    public void DrawSha(GroupMessageEventArgs e, string template) => Draw(e, template, null);

    [Command("draw <template> <qq>")]
    public static void Draw(GroupMessageEventArgs e, string template, User qq)
    {
        if (!drawTemplates.ContainsKey(template))
        {
    ...
    static readonly List<drawParamArray> paramList = new();
    static readonly DataTable dt = new();
    static readonly FontFamily font = new("HarmonyOS Sans SC Medium");
    static readonly StringFormat stf = new();
    static readonly SolidBrush brush = new(Color.Transparent);
    static readonly Pen pen = new(Color.Transparent);
    public static string AssetsPath = IntallkConfig.DataPath + "\\Resources\\";

    public static object Eval(string s) => dt.Compute(s, null);
    ...
    public static void Draw(string code, string oufile, params string[] param)
    {
        ...
        var g = Graphics.FromHwnd(IntPtr.Zero); var b = new Bitmap(1, 1);
    ...
    tryagain:
        ...
        try
        {
            ...
            {
                var pa = new drawParamArray
                {
                    name = param[j], key = param[j + 1],
                    width = (param[j + 1].Length * 20).ToString(),//g.MeasureString(param[j + 1], font).Width.ToString();
                    height = "20"  //g.MeasureString(param[j + 1], font).Height.ToString();
                };
                paramList.Add(pa);
                if (pa.name == "[msg]")
                {
                    ...
                    foreach (string content in param[j + 1].Split(' '))
                    {
                        ...
                        {
                            name = "[msg" + msgc + "]", key = content,
                            width = (content.Length * 20).ToString(),//g.MeasureString(param[j + 1], font).Width.ToString();
                            height = "20"
                        };
                        msgc++;
                        paramList.Add(pa);
                ...
                switch (p[0])
                {
                    ...
                    rewrite:
                        try
                        {
                            ...
                            var pos = new Point(Convert.ToInt32(FinalValue(p[1])), Convert.ToInt32(FinalValue(p[2])));
...
{
    static readonly Guid BlurEffectGuid = new("{633C80A4-1843-482B-9EF2-BE2834C5FDD4}");
    static readonly Guid UsmSharpenEffectGuid = new("{63CBF3EE-C526-402C-8F71-62C540BF5142}");
    ...
    static extern int GdipCreateEffect(Guid guid, out IntPtr effect);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipDeleteEffect(IntPtr effect);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipGetEffectParameterSize(IntPtr effect, out uint size);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipSetEffectParameters(IntPtr effect, IntPtr parameters, uint size);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipGetEffectParameters(IntPtr effect, ref uint size, IntPtr parameters);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipBitmapApplyEffect(IntPtr bitmap, IntPtr effect, ref Rectangle rectOfInterest, bool useAuxData, IntPtr auxData, int auxDataSize);

    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipBitmapCreateApplyEffect(ref IntPtr SrcBitmap, int numInputs, IntPtr effect, ref Rectangle rectOfInterest, ref Rectangle outputRect, out IntPtr outputBitmap, bool useAuxData, IntPtr auxData, int auxDataSize);


    // 这个函数我在C#下已经调用成功
    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipInitializePalette(IntPtr palette, int palettetype, int optimalColors, int useTransparentColor, int bitmap);

    // 该函数一致不成功，不过我在VB6下调用很简单，也很成功，主要是结构体的问题。
    [DllImport("gdiplus.dll", SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
    static extern int GdipBitmapConvertFormat(IntPtr bitmap, int pixelFormat, int dithertype, int palettetype, IntPtr palette, float alphaThresholdPercent);
    ...
    internal static TResult GetPrivateField<TResult>(this object obj, string fieldName)
    {
        ...
        var lfiFieldInfo = ltType.GetField(fieldName, System.Reflection.BindingFlags.GetField | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
    ...
    public static IntPtr NativeHandle(this Bitmap Bmp) => Bmp.GetPrivateField<IntPtr>("nativeImage");/*  用Reflector反编译System.Drawing.Dll可以看到Image类有如下的私有字段
